<script lang="ts">
  ...
  function onTouchStart(e: TouchEvent) {
    ...
    if (target && ( target.closest('.episodes-row') || target.closest('.seasons') || target.closest('.card') || target.closest('.season') || target.closest('button') )) {
      startX = null;
      startY = null;
      isScrolling = true;
      return;
  ...
  function onTouchMove(e: TouchEvent) {
    ...
    if (dragDirection === 'vertical' && dy > 0) {
      ...
      if (panelEl) panelEl.style.transform = `translateY(${finalTranslate}px)`;
      if (currentTranslate > 15) e.preventDefault();
    }
  }
  ...
  function onTouchEnd() {
    ...
    if (dragDirection === 'horizontal') {
      if (panelEl) {
        ...
        setTimeout(() => { if (panelEl) panelEl.style.transition = ''; }, 220);
      }
    ...
    <div class="episodes-row" role="list">
      {#each seasons[selectedSeason]?.items || [] as ep}
        ...
        >
          ...
          {:else if idx === currentItemIndex}
            <div class="badge-current">
              <Play class="h-3 w-3"/> Tocando
            </div>
          {/if}
